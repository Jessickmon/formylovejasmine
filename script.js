...
gsap.to('.text0',{
    x:'60%',rotate:10,duration:0.85,scrollTrigger:{
      trigger:'.text0',
      scrub:true,
      start:'top 20%'
    }
...
function random(min, max, float = false) {
  ...
  function startingX(idString) {
    const cloudWidth = parseInt(document.querySelector(idString).getAttribute('width'), 10);
    return window.innerWidth + cloudWidth; // Ensure the entire cloud is off-screen
  }
